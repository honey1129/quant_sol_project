...
**注意**：默认 ML 模式仅作为模拟盘基线，不代表已通过实盘上线门槛；信号不足时长期空仓属于正常的保守行为。
...
自动重训回答“候选模型能否替换旧模型”，严格 OOS 审计则回答“当前策略路线是否仍值得保留”。审计不会修改参数、替换模型或停止实时进程。

```bash
python -m run.strict_oos_validation
```

默认审计规则：

- 拉取 60 天训练历史和 90 天样本外数据，每 10 天滚动一折
- 每折只使用该折之前的数据重新训练，并保留 `MODEL_PURGE_BARS` 隔离区
- 测试折关闭阈值扫描，避免使用测试结果反向选择参数
- 同时运行不使用 ML 的简单趋势基线，并计入手续费、滑点和资金费
- 汇总最近 30/60/90 天表现，以及多空方向和入场行情环境归因

结论写入 `logs/strict_oos/latest.json` 和 `logs/strict_oos/latest.md`：

- `KEEP`：样本充分，并通过成本后收益、盈利因子、回撤、跨折稳定性及趋势基线门槛
- `WATCH`：平仓样本不足，只能继续观察，不能据此淘汰或上线新模型
- `ELIMINATE`：样本充分但稳健性门槛失败，应停止继续微调该策略路线
...
首次运行需要分页拉取较长历史数据并完成多折训练，耗时会明显高于普通回测。

### Directional V2 前向实验

旧策略被严格 OOS 审计淘汰后，新实验使用直接的 `flat / long / short` 三分类标签：信号在当前 K 线收盘后生成，标签从下一根开盘分别模拟多头和空头，扣除双边手续费与滑点后选择方向。趋势和 regime 仅作为模型特征，不再硬编码交易方向。

冻结规范位于 `research/experiments/directional_v2/spec.json`，并由 `spec.sha256` 防止静默修改。实验模型保存在 `models/experiments/directional_v2/`，不会被实时监控或自动重训加载。

```bash
# 查看规范完整性和留出状态
python -m run.directional_v2_experiment

# 仅允许在留出开始前执行一次；当前冻结模型已经生成
python -m run.train_directional_v2

# 留出期内只返回 collecting 状态；达到时间和样本门槛后才执行最终评估
python -m run.evaluate_directional_v2_forward
```

最终留出从 `2026-07-23T00:00:00Z` 开始，最早评估时间为 `2026-08-22T00:00:00Z`，且至少需要 30 笔平仓。在此之前，开发集指标不能作为上线依据；任何标签、信号、模型、执行或门槛变化都必须创建新的实验编号并重新开始留出计时。
...
实时风控优先使用 WebSocket；缓存过期时以 1 秒超时、1 次重试快速降级到只读 REST，下单与常规数据读取仍保留完整重试。超过 `RISK_LOOP_WARN_SEC` 会记录 `risk_loop_latency`；OCO 保护不依赖本地循环。

成交审计与日报记录触发延迟、确认耗时及滑点。

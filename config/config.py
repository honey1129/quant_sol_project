...
# 硬门槛可以通过环境变量提高，但重训流程会强制保留 1 笔交易和 PF > 1 的绝对底线。
MODEL_RETRAIN_HARD_MIN_PROFIT_FACTOR = float(os.getenv("MODEL_RETRAIN_HARD_MIN_PROFIT_FACTOR", 1.0))
...
# KILL_SWITCH_FILE：该文件存在时立即停止所有新开仓。
KILL_SWITCH_FILE = os.getenv("KILL_SWITCH_FILE", "logs/kill_switch.flag")
